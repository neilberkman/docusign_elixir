...
defmodule DocuSign.RequestBuilder do
  ...
  def add_optional_params(request, definitions, [{key, value}] = values) do
    case definitions do
      ...
      _ ->
        _add_optional_params(request, definitions, values)
    end
  end

  def add_optional_params(request, definitions, values),
    do: _add_optional_params(request, definitions, values)

  defp _add_optional_params(request, _, []), do: request

  defp _add_optional_params(request, definitions, [{key, value} | tail]) do
    case definitions do
      %{^key => location} ->
        request
        |> add_param(location, key, value)
        |> _add_optional_params(definitions, tail)

      _ ->
        _add_optional_params(request, definitions, tail)
    end
  end
