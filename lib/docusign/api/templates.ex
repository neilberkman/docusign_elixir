...
defmodule DocuSign.Api.Templates do
  ...
  - `opts` (keyword): Optional parameters
    ...
    - `:shared_by_me` (String.t): When **true,** the response only includes templates shared by the user. When **false,** the response only returns template not shared by the user. If not specified, templates are returned whether or not they have been shared by the user.
    ...
    - `:user_filter` (String.t): Filters the templates in the response. Valid values are:   - `owned_by_me`: Results include only templates owned by the user. - `shared_with_me`: Results include only templates shared with the user.   - `all`:  Results include all templates owned or shared with the user.
