...
defmodule DocuSign.Api.Users do
  ...
  - `opts` (keyword): Optional parameters
    ...
    - `:alternate_admins_only` (String.t): When **true,** returns only alternate administrators. These users are not administrators but will be set as such if all administrator memberships are closed. The default value is **false.**
    ...
    - `:domain_users_only` (String.t): When **true,** return only users in [domains](https://support.docusign.com/s/document-item?rsc_301=&bundleId=rrf1583359212854&topicId=jub1589318086105.html) claimed by your organization. The default value is **false.**
    ...
    - `:email_substring` (String.t): Filters results based on a fragment of an email address. For example, you could enter `gmail.com` to return all users who have Gmail addresses.  **Note:** You do not use a wildcard character with this parameter. You can use either this parameter or the `email` parameter, but not both. 
