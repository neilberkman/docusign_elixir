...
- **HTTP Client Migration**: Replace Tesla with Req HTTP client
  - All API responses now return `Req.Response` instead of `Tesla.Env`
  - Connection module completely rewritten to use Req
  ...
  - Any code directly accessing `Tesla.Env` structures will need updates
